  ...
  return (
    <aside className="w-full h-full overflow-y-auto bg-white border-r border-surface-200">
        ...
        <div className="grid grid-cols-3 gap-2">
          {Object.values(TEMPLATES).map((t) => (
            <button
              ...
              className={`rounded-lg border-2 p-2.5 text-center text-xs font-medium transition-all ${
      ...
      <Section title="Personal Info" defaultOpen={true}>
        ...
        <div className="flex items-end gap-2">
          <div className="flex-1">
            <Field label="Pronouns" value={data.pronouns} onChange={(v) => setField("pronouns", v)} placeholder="e.g. she/her" />
          </div>
          <div className="pb-0.5">
            <Toggle label="" checked={data.showPronouns} onChange={(v) => setField("showPronouns", v)} />
          </div>
        </div>
      </Section>
      ...
      <Section title="Contact Details">
        ...
        <div className="flex items-end gap-2">
          <div className="flex-1">
            <Field label="Address" value={data.address} onChange={(v) => setField("address", v)} placeholder="Optional" />
          </div>
          <div className="pb-0.5">
            <Toggle label="" checked={data.showAddress} onChange={(v) => setField("showAddress", v)} />
          </div>
        </div>
      </Section>
        ...
        {data.showHeadshot && (
          <>
            ...
            <SelectField
              ...
              onChange={(v) => setField("headshotShape", v as "circle" | "square" | "rounded")}
              options={[
          ...
          <div className="flex flex-wrap gap-1.5">
            {Object.entries(COLOR_THEMES).map(([key, theme]) => (
              <button
                key={key}
                type="button"
                onClick={() => setFields(theme)}
                className="flex items-center gap-1.5 rounded-md border border-surface-200 px-2 py-1 text-xs text-surface-600 hover:border-surface-300 transition-colors"
              >
        ...
        {data.showDivider && (
          <>
            <SelectField
              ...
              onChange={(v) => setField("dividerStyle", v as "solid" | "dashed" | "dotted" | "none")}
              options={[
