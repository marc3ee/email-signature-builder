...
// Minimal inline SVG data URIs for social icons (Gmail-safe as hosted PNGs,
// but for the builder preview we use inline SVGs encoded as data URIs).
// In production, replace these with hosted PNG URLs on your CDN.

const SOCIAL_ICONS: Record<string, { url: (data: SignatureData) => string; svg: string }> = {
  linkedin: {
    url: (d) => d.linkedin,
...
export function renderSocialIconsHtml(data: SignatureData): string {
  ...
  const icons = Object.entries(SOCIAL_ICONS)
    ...
    .map(([, config]) => {
      ...
      // Encode SVG as data URI with the user's chosen icon color
      const coloredSvg = config.svg.replace(/currentColor/g, data.iconColor);
      const encoded = `data:image/svg+xml,${encodeURIComponent(coloredSvg)}`;
      return `<a href="${url}" target="_blank" style="text-decoration: none; margin-right: 6px;"><img src="${encoded}" width="${data.iconSize}" height="${data.iconSize}" style="display: inline-block; vertical-align: middle;" /></a>`;
    });
