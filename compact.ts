...
export function renderCompact(data: SignatureData): string {
  ...
  if (data.website) {
    ...
    line2Parts.push(`<a href="${data.website}" style="${s.link}">${label}</a>`);
  }
  ...
  const ctaHtml = data.ctaText && data.ctaUrl
    ? `<tr><td style="padding-top: 8px;"><a href="${data.ctaUrl}" target="_blank" style="display: inline-block; padding: 5px 14px; background-color: ${data.ctaBgColor}; color: ${data.ctaTextColor}; text-decoration: none; border-radius: 3px; font-size: ${data.fontSize - 1}px; font-family: ${data.fontFamily};">${data.ctaText}</a></td></tr>`
    : "";
